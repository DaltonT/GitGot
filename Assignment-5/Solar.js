...
var far = 400;      // far clipping plane's distance
...
function render() {
  ...
  ms.push();
  ms.rotate(Math.PI*(1/data.year)*time, [0, 1, 0]);
  ...
  ms.push();
  ms.rotate(Math.PI*(1/SolarSystem["Earth"].year)*time, [0, 1, 0]);
  ms.translate(SolarSystem["Earth"].distance*10, 0, 0);
  
  ms.rotate(Math.PI*(1/data.year)*time, [0, 1, 0]);
  ms.translate(SolarSystem["Earth"].radius+data.distance*250, 0, 0);
  ms.scale(data.radius);
